...
void printtle
   ...
   )
   {
     ...
     sprintf( longstr1, "1 %5ldN%10s%02i%12.8f  .00000000  00000-0 ",
     ...
     sprintf( longstr2, "2 %5ld %08.4f%9.4f %07.0f %8.4f %8.4f %11.8f 00000\n",
...
//                  satrec.a     = pow( 1.0 / (satrec.no * satrec.no * 13.446839 * 13.446839) , 1.0 / 3.0 );  // er
                  ...
                  if (satrec.a < 1.0)  // can't be less than the radius of the earth
                    {
                      printf("changed a %11.7f ", satrec.a);
                  ...
                  if (satrec.no < 1.0e-5)
                    {
                      printf("changed no %11.7f ", satrec.no);
                  ...
                  if (satrec.ecco > 1.0)
                    {
                      printf("changed ecco %11.7f ", satrec.ecco);
...
void findatwaatwb
   ...
   )
   {
     senrec currsenrec;
     obsrec currobsrec;
     int rowc, colc, r, c, i, j;
     double rs[3], r3[3], v3[3],
             ...
             if ( fabs(dx[i][0]) > fabs(0.4 * xnom[i]) )  // 0.40   .8xx seem to be the same
               {
                 if (fabs(dx[i][0]/xnom[i]) > 1.0)
                   {
//                       printf("z %1i delta %11.7f %11.7f  %11.7f ",i, xnom[i],dx[i][0]/xnom[i], dx[i][0]);
...
//                     dx[i][0] = 0.10 * xnom[i] * sgn(dx[i][0]);   // 0.90 0.1xx try leaving the same
                   ...
                   {
//                       printf("y %1i delta %11.7f %11.7f  %11.7f ",i, xnom[i],dx[i][0]/xnom[i], dx[i][0]);
...
//scanf( "%i",&statesize );
//printf("a %1i delta %11.7f %11.7f  %11.7f \n",i, xnom[i],dx[i][0]/xnom[i], dx[i][0]);
               }
*/
             if (statetype != 'v')
               {
